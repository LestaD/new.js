...
const createLocation = answers =>
  resolve(process.cwd(), changeCase.paramCase(answers.name))
...
const questions = conf => make({
  ...
  repo: { // https://github.com/npm/init-package-json/blob/08d1575cf2c2adae894980fca7df021fb44565f9/default-input.js#L184
    message: 'Repository:',
    default: (answers) => {
      const [scope, name] = answers.name.split('/')
      if (name) {
  ...
  apptype: { // https://github.com/npm/init-package-json/blob/08d1575cf2c2adae894980fca7df021fb44565f9/default-input.js#L129
    type: 'checkbox',
...
module.exports =
async function libQuestions(initialConfig) {
  // eslint-disable-next-line no-console
  console.log(chalk.gray('[new.js] '), chalk.blue('Let\'s create new package!\n'))

  const answersRaw = await inquirer.prompt(questions(initialConfig))
  const answers = resolveAnswers(initialConfig, answersRaw)
